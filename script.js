...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- V18.4: Save/Load Functions ---
    /** V18.4: บันทึกข้อมูล shops array ลง localStorage */
    ...
    /** V18.4: โหลดข้อมูล shops array จาก localStorage */
    ...
    // --- V18.4: Rendering Functions ---

    /** V18.4: วาดแถบแท็บร้านค้าใหม่ทั้งหมด */
    ...
    /** V18.4: วาดเนื้อหาของแท็บที่ถูกเลือก (เพิ่มปุ่ม Clear Items) */
    function renderTabContent() {
        ...
        if (activeShop) { console.log("Rendering content for shop:", activeShop.name); const headerDiv = document.createElement('div'); headerDiv.className = 'shop-header'; const shopNameDisplay = document.createElement('span'); shopNameDisplay.className = 'shop-name-display'; shopNameDisplay.textContent = activeShop.name; headerDiv.appendChild(shopNameDisplay); tabContentArea.appendChild(headerDiv); const entryArea = createItemEntryArea(activeShop.id); tabContentArea.appendChild(entryArea); const itemListArea = document.createElement('div'); itemListArea.className = 'item-list-area'; itemListArea.id = `item-list-${activeShop.id}`; const ul = document.createElement('ul'); if (activeShop.items.length > 0) { activeShop.items.forEach((item, index) => { ul.appendChild(createShopItemRow(activeShop.id, item, index)); }); } else { ul.innerHTML = '<li class="item-list-placeholder">ยังไม่มีรายการในร้านนี้...</li>'; } itemListArea.appendChild(ul); tabContentArea.appendChild(itemListArea);
            ...
            clearBtn.className = 'action-button clear-items-btn'; // Class ใหม่
            clearBtn.type = "button";
            clearBtn.dataset.shopId = activeShop.id; // ใส่ ID ร้านไว้ที่ปุ่ม
            // Listener จะใช้ delegation

            // ปุ่มสรุป/แก้ไข
            const summarizeBtn = document.createElement('button');
            summarizeBtn.textContent = '📋 สรุปรายการร้านนี้'; // กลับไปใช้ข้อความเดิม
            summarizeBtn.className = 'action-button summarize-btn';
            summarizeBtn.type = "button";
            summarizeBtn.addEventListener('click', () => showSummary(activeShopId));

            buttonsDiv.appendChild(clearBtn); // เพิ่มปุ่มล้างก่อน
            buttonsDiv.appendChild(summarizeBtn); // ตามด้วยปุ่มสรุป
    ...
    /** V18.4: สร้างแถวสำหรับแสดงรายการสินค้าใน List Area */
    function createShopItemRow(shopId, itemData, index) { /* (เหมือนเดิมจาก V18) */
    ...
    // --- V18.4: Event Handlers (เพิ่ม handleClearShopItems) ---

    /** V18.4: จัดการการคลิกแท็บร้านค้า */
    ...
    /** V18.4: จัดการการคลิกปุ่ม + เพื่อเพิ่มร้านใหม่ */
    ...
    /** V18.4: จัดการการยกเลิกเพิ่มร้านใหม่ */
    ...
    /** V18.4: แสดงสถานะการเพิ่ม/ซ้ำ ใน Entry Area */
    ...
    /** V18.4: จัดการการกดปุ่มแก้ไข (ดินสอ) ใน List Area */
    ...
    /** V18.4: จัดการการกดปุ่มบันทึก (เครื่องหมายถูก) ในโหมดแก้ไข Inline */
    ...
    /** V18.4: จัดการการกดปุ่มยกเลิก (X) ในโหมดแก้ไข Inline */
    ...
    /** V18.4: จัดการการกดปุ่ม "ล้างรายการร้านนี้" (Delegated) */
    function handleClearShopItems(event) {
        ...
        if (confirm(`**คำเตือน!** ยืนยันล้างรายการของทั้งหมดในร้าน "${shop.name}" จริงดิ?\nร้านค้าจะยังอยู่ แต่ของจะหายหมดนะ!`)) {
            shop.items = []; // ล้าง array items ของร้านนี้ใน state
            console.log(`Cleared items for shop ${shopId}`);
            saveShopsToLocalStorage(); // เซฟการเปลี่ยนแปลง
            renderTabContent(); // วาดเนื้อหาของแท็บนี้ใหม่ (จะแสดงว่าไม่มีรายการ)

            // แสดงข้อความยืนยัน (อาจจะใช้ showEntryStatus)
    ...
    /** V18.4: ดึงข้อมูลจาก State Array ('shops') สำหรับสร้างสรุป */
    ...
    function showSummary(shopId = null) { /* (เหมือนเดิมจาก V18.2) */
        ...
        console.log("V18.4 - Attempting to display modal..."); summaryModal.style.display = 'block'; console.log("V18.4 - Modal display style set to 'block'.");
    }
    ...
    async function initializeApp() {
        console.log("--- เริ่มต้น initializeApp (V18.4) ---");
        if (!loadingErrorDiv) { console.error("หา #loading-error-message ไม่เจอ!"); return; }

        // V18.4: โหลดข้อมูลร้านค้าจาก localStorage ก่อน
        ...
        if (!loadedFromStorage) {
            ...
            // V18.4: ถ้าใช้ข้อมูลเริ่มต้นครั้งแรก ให้เซฟลง localStorage ด้วยเลย
        ...
        try {
            ...
            setupEventListeners();
            console.log("--- initializeApp เสร็จสิ้น (V18.4) ---");
        }
    }

    /** V18.4: ฟังก์ชันรวมการตั้งค่า Event Listeners หลัก */
    function setupEventListeners() {
        console.log("Setting up event listeners...");

        // --- Listeners จาก V17/V18 ---
        ...
        if (tabContentArea) {
            ...
            // V18.4: เพิ่ม listener สำหรับปุ่ม Clear Items
